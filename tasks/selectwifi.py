...
def run():
    ...
    selected: int = ui.menu(options, height=20, title="Select WiFi Interface")
    
    if selected == len(options) - 1:
        return
    
    file.write("data/wifi.interface", netinterfaces[selected][0])
    ui.msgBox(f"Selected WiFi interface: {netinterfaces[selected][0]}", "Setting Queue")
    print(f"Selected WiFi interface: {netinterfaces[selected][0]}")
